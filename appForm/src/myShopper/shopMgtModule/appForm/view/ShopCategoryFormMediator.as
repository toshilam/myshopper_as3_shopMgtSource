package myShopper.shopMgtModule.appForm.view 
{
	...
	import myShopper.fl.shopMgt.form.ShopMgtCategoryForm;
	...
	public class ShopCategoryFormMediator extends ApplicationMediator implements IContainerMediator, ITabFormMediator
	{
		...
		private var form:ShopMgtCategoryForm;
		private var _window:BaseWindow;
		//private var _bg:ApplicationDisplayObject;
		
		public function ShopCategoryFormMediator(inMediatorName:String = null, inViewComponent:IModuleMain = null) 
		{
		...
		override public function listNotificationInterests():Array 
		{
			...
			if (mediatorName == MediatorID.SHOP_MGT_CATEGORY_CREATE)
			{
				arr = [NotificationType.ADD_FORM_SHOP_CATEGORY, NotificationType.CREATE_CATEGORY_FAIL, NotificationType.CREATE_CATEGORY_SUCCESS];
			}
			else if (mediatorName == MediatorID.SHOP_MGT_CATEGORY_UPDATE)
			{
				arr = [NotificationType.ADD_FORM_SHOP_CATEGORY, NotificationType.UPDATE_CATEGORY_FAIL, NotificationType.UPDATE_CATEGORY_SUCCESS];
			}
		...
		override public function handleNotification(note:INotification):void 
		{
			...
			switch(note.getName())
			{
				case NotificationType.ADD_FORM_SHOP_CATEGORY:
				{
					...
					if (_window)
					{
						...
						form = _window.addApplicationChild(vo.displayObject, null) as ShopMgtCategoryForm;
				...
				{
					createFormAlert(getMessage(MessageID.SUCCESS_SAVE), form.txtName);
					//startButtonListener();
					break;
				}
				case NotificationType.CREATE_CATEGORY_FAIL:
				case NotificationType.UPDATE_CATEGORY_FAIL:
				{
			...
			switch(e.targetButton)
			{
				...
				case form.btnSend:
				{
					...
					if (result === true)
					{
						...
						if (mediatorName == MediatorID.SHOP_MGT_CATEGORY_CREATE)
						{
							sendNotification(ShopMgtEvent.SHOP_CREATE_CATEGORY);
						}
						else if (mediatorName == MediatorID.SHOP_MGT_CATEGORY_UPDATE)
						{
							sendNotification(ShopMgtEvent.SHOP_UPDATE_CATEGORY);
						}
					}
					else
					{
