package myShopper.shopMgtModule.appForm.view 
{
	...
	public class ShopNewsAlertMediator extends ApplicationMediator implements IContainerMediator 
	{
		private var _alert:ApplicationDisplayObject;
		
		public function ShopNewsAlertMediator(inMediatorName:String = null, inViewComponent:IModuleMain = null) 
		{
		...
		{
			return [NotificationType.ADD_ALERT_SHOP_NEWS, NotificationType.DELETE_NEWS_FAIL, NotificationType.DELETE_NEWS_SUCCESS];
		}
		...
		override public function handleNotification(note:INotification):void 
		{
			...
			switch(note.getName())
			{
				case NotificationType.ADD_ALERT_SHOP_NEWS:
				{
					_alert = Alert.show
					(
						new AlerterVO
						(
							...
							getMessage(myShopper.shopMgtCommon.emun.MessageID.NEWS_DELETE, 'string', AssetLibID.XML_LANG_SHOP_MGT)
						)
					)
				...
				case NotificationType.DELETE_NEWS_FAIL:
				case NotificationType.DELETE_NEWS_SUCCESS:
				{
					var result:ResultVO = note.getBody() as ResultVO;
					
					var title:String = note.getName() == NotificationType.DELETE_NEWS_FAIL ? getMessage(MessageID.ERROR_TITLE) : getMessage(MessageID.SUCCESS_TITLE);
					var message:String = note.getName() == NotificationType.DELETE_NEWS_FAIL ? getMessage(MessageID.ERROR_GET_DATA) + '\nCODE:(' + result.code + ')\n' + getMessage(MessageID.CONTACT_US) : getMessage(myShopper.shopMgtCommon.emun.MessageID.NEWS_DELETE_SUCCESS, 'string', AssetLibID.XML_LANG_SHOP_MGT);
		...
		private function alertEventHandler(e:AlerterEvent):void 
		{
			...
			{
				sendNotification(ShopMgtEvent.SHOP_DELETE_NEWS);
			}
			else
			{
				sendNotification(WindowEvent.CLOSE, mediatorName);
