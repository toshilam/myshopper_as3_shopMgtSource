package myShopper.shopMgtModule.appForm.view 
{
	...
	public class ShopProductAlertMediator extends ApplicationMediator implements IContainerMediator 
	{
		private var _alert:ApplicationDisplayObject;
		
		public function ShopProductAlertMediator(inMediatorName:String = null, inViewComponent:IModuleMain = null) 
		{
		...
		{
			return [NotificationType.ADD_ALERT_SHOP_PRODUCT, NotificationType.DELETE_PRODUCT_FAIL, NotificationType.DELETE_PRODUCT_SUCCESS];
		}
		...
		override public function handleNotification(note:INotification):void 
		{
			...
			switch(note.getName())
			{
				case NotificationType.ADD_ALERT_SHOP_PRODUCT:
				{
					_alert = Alert.show
					(
						new AlerterVO
						(
							...
							getMessage(myShopper.shopMgtCommon.emun.MessageID.PRODUCT_DELETE, 'string', AssetLibID.XML_LANG_SHOP_MGT)
						)
					)
				...
				case NotificationType.DELETE_PRODUCT_FAIL:
				case NotificationType.DELETE_PRODUCT_SUCCESS:
				{
					var result:ResultVO = note.getBody() as ResultVO;
					
					var title:String = note.getName() == NotificationType.DELETE_PRODUCT_FAIL ? getMessage(MessageID.ERROR_TITLE) : getMessage(MessageID.SUCCESS_TITLE);
					var message:String = note.getName() == NotificationType.DELETE_PRODUCT_FAIL ? getMessage(MessageID.ERROR_GET_DATA) + '\nCODE:(' + result.code + ')\n' + getMessage(MessageID.CONTACT_US) : getMessage(myShopper.shopMgtCommon.emun.MessageID.PRODUCT_DELETE_SUCCESS, 'string', AssetLibID.XML_LANG_SHOP_MGT);
		...
		private function alertEventHandler(e:AlerterEvent):void 
		{
			...
			{
				sendNotification(ShopMgtEvent.SHOP_DELETE_PRODUCT);
			}
			else
			{
				sendNotification(WindowEvent.CLOSE, mediatorName);
