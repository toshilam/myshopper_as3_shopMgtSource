package myShopper.shopMgtModule.appForm.model
{
	...
	public class ShopPrintFormProxy extends ApplicationRemoteProxy//ApplicationProxy implements IRemoteDataProxy
	{
		...
		public function ShopPrintFormProxy(inName:String, inData:Object)
		{
			super( inName, inData );
		}
		...
		override public function onRegister():void
		{
			...
			_targetNode = windowNode.*.(@id == AssetID.BTN_SHOP_S_PRINT)[0];
		}
			...
			if (inAsset is Notification)
			{
				...
				if (classID)
				{
					...
					(
						...
						new DisplayObjectVO(classID, assetManager.getData(classID, AssetLibID.AST_SHOP_MGT_FORM), _targetNode, _userInfo.printerVO) 
					);
					
					//always search from google as user may have added a new printer
					getRemoteData(AMFGoogleServicesType.PRINTER_SEARCH);
				}
				else
				{
		...
		private function getAssetIDByNoteType(inID:String):String
		{
			switch(inID)
			{
				case ShopMgtEvent.SHOP_SET_PRINT: return AssetClassID.FORM_SHOP_PRINT;
			}
