package myShopper.shopMgtModule.appForm.model
{
	...
	public class ShopLogoFormProxy extends ApplicationRemoteProxy// ApplicationProxy implements IRemoteDataProxy
	{
		...
		public function ShopLogoFormProxy(inName:String, inData:Object)
		{
			super( inName, inData );
		}
		...
		override public function onRegister():void
		{
			...
			_targetNode = windowNode.*.(@id == AssetID.BTN_SHOP_S_LOGO)[0];
		}
			...
			if (inAsset is Notification)
			{
				...
				if (classID)
				{
					...
					if (_shopInfoVO)
					{
						...
						sendNotification
						(
							NotificationType.ADD_FORM_SHOP_LOGO, 
							new DisplayObjectVO(classID, assetManager.getData(classID, AssetLibID.AST_SHOP_MGT_FORM), _targetNode, _shopInfoVO.logoFileVO) 
						);
					}
					else
					{
		...
		private function getAssetIDByNoteType(inID:String):String
		{
			switch(inID)
			{
				case ShopMgtEvent.SHOP_UPDATE_LOGO: return AssetClassID.FORM_SHOP_LOGO;
			}
		...
		{
			if (inService == AMFShopManagementServicesType.UPDATE_LOGO)
			{
				...
				if (userInfo && userInfo.isLogged && inData is FileImageVO)
				{
					...
					else
					{
						...
						sendNotification(NotificationType.UPDATE_LOGO_FAIL);
						return false;
					}
			...
			if (inService == AMFShopManagementServicesType.UPDATE_LOGO)
			{
				...
				if (resultVO)
				{
					if ( resultVO.code != AMFServicesErrorID.NONE )
					{
						...
						sendNotification(NotificationType.UPDATE_LOGO_FAIL, resultVO);
						return false;
					}
					else
					{
						if (_shopVOService.setShopLogoByVO(_newImageVO))
						{
							sendNotification(NotificationType.UPDATE_LOGO_SUCCESS);
						}
						else
						{
