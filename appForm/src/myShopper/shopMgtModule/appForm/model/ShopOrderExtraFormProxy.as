package myShopper.shopMgtModule.appForm.model
{
	...
	public class ShopOrderExtraFormProxy extends ApplicationProxy /*implements IRemoteDataProxy*/
	{
		...
		public function ShopOrderExtraFormProxy(inName:String, inData:Object)
		{
			super( inName, inData );
		}
		...
		private var _selectedOrderVO:IVO;
		...
		override public function onRegister():void
		{
			...
			_targetNode = windowNode.*.(@id == AssetID.SHOP_ORDER_EXTRA)[0];
		}
		...
		override public function initAsset(inAsset:Object = null):void 
		{
			if (inAsset is Notification)
			{
				...
				_selectedOrderVO = Notification(inAsset).getBody() as IVO;
				
				if (classID && _selectedOrderVO)
				{
					_orderExtraVO = new ShopOrderExtraVO(_selectedOrderVO.id);
					
					sendNotification
					(
						proxyName == ProxyID.SHOP_MGT_ORDER_EXTRA ? NotificationType.ADD_FORM_SHOP_ORDER_EXTRA : NotificationType.ADD_FORM_SHOP_SALES_EXTRA, 
						new DisplayObjectVO(classID, assetManager.getData(classID, AssetLibID.AST_COMMON), _targetNode, _orderExtraVO) 
					);
		...
		/*public function getRemoteData(inService:String, inData:Object = null):Boolean
		{
			...
			if (userInfo && userInfo.isLogged && orderVO)
			{
			...
			if (resultVO)
			{
				...
				else
				{
					if (inService == AMFShopManagementServicesType.GET_ORDER_PRODUCT)
					{
						...
						{
							sendNotification(NotificationType.RESULT_GET_ORDER_PRODUCT);
