package myShopper.shopMgtModule.appForm.model
{
	...
	public class ShopProductAlertProxy extends ApplicationRemoteProxy //ApplicationProxy implements IRemoteDataProxy
	{
		...
		public function ShopProductAlertProxy(inName:String, inData:Object)
		{
			super( inName, inData );
		}
		...
		override public function initAsset(inAsset:Object = null):void 
		{
			if (inAsset is Notification)
			{
				...
				if (type == ShopMgtEvent.SHOP_DELETE_PRODUCT)
				{
					_productVO = note.getBody() as ShopProductFormVO;
					
					if (_productVO)
					{
						sendNotification(NotificationType.ADD_ALERT_SHOP_PRODUCT);
					}
					else
					{
		...
		override public function getRemoteData(inService:String, inData:Object = null):Boolean
		{
			...
			if (userInfo && userInfo.isLogged && _productVO)
			{
				if (inService == AMFShopManagementServicesType.DELETE_PRODUCT)
				{
					//p_category for retrieving list of product on server side
					requestData = 	{ p_user_id:userInfo.uid, p_no:_productVO.productNo, p_id:_productVO.productID, p_category:_productVO.productCategoryNo };
				}
				else
				{
		...
		override public function setRemoteData(inService:String, inData:Object):Boolean
		{
			...
			if (resultVO)
			{
				if ( resultVO.code != AMFServicesErrorID.NONE )
				{
					...
					sendNotification(NotificationType.DELETE_PRODUCT_FAIL, resultVO);
				...
				else
				{
					...
					var categoryVO:ShopCategoryFormVO = _shopInfoVO.productCategoryList.getVOByCategoryNo(_productVO.productCategoryNo) as ShopCategoryFormVO;
					
					if( categoryVO && categoryVO.productList.removeVO(_productVO) )
					//in case of DELETE, no need to reset the list of vo
					//if ( _shopVOService.setShopCategory(resultVO.result) )
					{
						sendNotification(NotificationType.DELETE_PRODUCT_SUCCESS);
					}
					else
					{
