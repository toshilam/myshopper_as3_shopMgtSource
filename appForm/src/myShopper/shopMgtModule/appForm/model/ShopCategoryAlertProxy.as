package myShopper.shopMgtModule.appForm.model
{
	...
	public class ShopCategoryAlertProxy extends ApplicationRemoteProxy//ApplicationProxy implements IRemoteDataProxy
	{
		...
		public function ShopCategoryAlertProxy(inName:String, inData:Object)
		{
			super( inName, inData );
		}
		...
		override public function initAsset(inAsset:Object = null):void 
		{
			if (inAsset is Notification)
			{
				...
				if (type == ShopMgtEvent.SHOP_DELETE_CATEGORY)
				{
					_categoryVO = note.getBody() as ShopCategoryFormVO;
					
					if (_categoryVO)
					{
						sendNotification(NotificationType.ADD_ALERT_SHOP_CATEGORY);
					}
					else
					{
		...
		override public function getRemoteData(inService:String, inData:Object = null):Boolean
		{
			...
			if (userInfo && userInfo.isLogged && _categoryVO)
			{
				if (inService == AMFShopManagementServicesType.DELETE_CATEGORY)
				{
					requestData = 	{ c_user_id:userInfo.uid, c_no:_categoryVO.categoryNo };
				}
				else
				{
		...
		override public function setRemoteData(inService:String, inData:Object):Boolean
		{
			...
			if (resultVO)
			{
				if ( resultVO.code != AMFServicesErrorID.NONE )
				{
					...
					sendNotification(NotificationType.DELETE_CATEGORY_FAIL, resultVO);
				...
				else
				{
					...
					if( _shopInfoVO.productCategoryList.removeVO(_categoryVO) )
					...
					{
						sendNotification(NotificationType.DELETE_CATEGORY_SUCCESS);
					}
					else
					{
